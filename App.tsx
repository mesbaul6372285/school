...
const AdminDashboard: React.FC<{ currentUser: Member; setCurrentUser: (u: Member) => void }> = ({ currentUser, setCurrentUser }) => {
  ...
  const openCmsModal = (type: 'PROJ' | 'ANN' | 'GAL' | 'FACT', data?: any) => {
      ...
      if (data) {
          initialData = { ...data };
          if (type === 'PROJ') {
              initialData.researchers = Array.isArray(data.researchers) ? data.researchers.join(', ') : data.researchers;
          }
      } else {
          // Creating New: Default Templates
          if (type === 'PROJ') initialData = { id: Date.now(), title: '', status: 'ONGOING', image: '', description: '', researchers: '' };
          if (type === 'ANN') initialData = { id: Date.now(), title: '', date: new Date().toISOString().split('T')[0], type: 'PUBLIC', priority: 'NORMAL', description: '' };
          if (type === 'GAL') initialData = { id: Date.now(), title: '', category: 'Event', image: '' };
      }
  ...
  const handleSaveCms = (e: React.FormEvent) => {
      ...
      if (type === 'PROJ') {
          const exists = projects.find(p => p.id === finalData.id);
          let newList;
...
const LoginView: React.FC<{ onSuccess: (user: Member) => void }> = ({ onSuccess }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!isAdminMode && user.role === 'ADMIN') {
       // Allow admins to login on member form, or block? 
       // Usually better to redirect or allow. Let's allow but maybe warn.
    }
...
const App: React.FC = () => {
  ...
  const [weeklyFact, setWeeklyFact] = useState<WeeklyFact>(DB.getWeeklyFact());
  ...
  return (
    <div className="min-h-screen bg-[#050b14] text-gray-100 flex flex-col font-sans selection:bg-cyan-500/30 selection:text-cyan-200">
      ...
      <main className="flex-grow z-10 relative">
        ...
        {/* Unified Login View handled by LOGIN_MEMBER route, but also mapped LOGIN_ADMIN to it for simplicity if redirected */}
        ...
        {currentView === View.DASHBOARD_ADMIN && currentUser && <AdminDashboard currentUser={currentUser} setCurrentUser={setCurrentUser} />}
      </main>
